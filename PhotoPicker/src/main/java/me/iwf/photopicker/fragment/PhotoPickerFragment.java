...
public class PhotoPickerFragment extends Fragment {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        showCamera = getArguments().getBoolean(EXTRA_CAMERA, true);
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == ImageCaptureManager.REQUEST_TAKE_PHOTO && resultCode == RESULT_OK) {
            ...
            if (directories.size() > 0) {
                ...
                ArrayList<String> carmerPhotos = new ArrayList<>();
                carmerPhotos.add(path);
                intent.putStringArrayListExtra(KEY_SELECTED_PHOTOS, carmerPhotos);
                getActivity().setResult(RESULT_OK, intent);
                getActivity().finish();

            }
        } else if (requestCode == ImageCaptureManager.REQUEST_TAKE_PHOTO && resultCode == RESULT_CANCELED) {
            getActivity().finish();
        }
    }
